...
#define MAX 2.30103 /* limite maximo FIXO da energia ou beta */
#define MIN 0.01011 /* limite minimo FIXO da energia ou beta */
...
{
  long int i,k,n[M]={0};
  double x,y,r,energy,min=MIN,max=MAX,media[M]={0}; /* min e max fixos */
   
  for(i=0;i<N;i++)
    if(v[i][0]>=min && v[i][0]<=max && v[i][0]*v[i][1]) {
