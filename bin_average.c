...
/* velocidades de Fermi em unidades de c */
#define Bf_Xe 0.0031590
#define Bf_Ar 0.0029055
#define Bf_Si 0.0071084
#define Bf_Ge 0.0069

/* velocidades limite */
#define B_1keV 0.00146
#define B_10keV 0.00462
#define B_a_M 0.00073

#define M 500     /* numero de pontos de output */
#define MAX 0.2 /* limite maximo da energia ou beta (o max pode ser menor) */
#define MIN 1     /* energia minima ou beta apenas como valor de referencia inicial (o min devera ser menor) */
...
#define RO d_Pb  /* densidade do material do alvo */

/* constantes */
#define NA 6.022e23 /* numero de Avogrado em unidades /mole */

void joao_viana(FILE *fp,double **v,long int N);
...
{
  char f_out[50],*name="_bin_average.dat";
  FILE *fp_in,*fp_out;
  int n,j=0,l;
  long int N,i,ni=0;
  double **v;
...
{
  long int i,k,n[M]={0};
  double x,y,r,energy,min=MIN,max=0,media[M]={0};
  ...
  for(k=0;k<M;k++)
    if(media[k]){
      energy=min+(double)k*(max-min)/(M-1);
      if(energy) {
